		...
		const handleKeyboardPress = (e) => {
			if (e.keyCode === 13) {
				...
				case constants.WALLET_033X:
					// Logic for WALLET_033X and WALLET_rivineG is almost identical
				...
				case constants.WALLET_rivineG:
					// Use the WALLET_rivineG options if we didn't fall through from WALLET_033X.
					options = options || {
						'keyfiles': getArgumentString(this.props.currentCommand, constants.specialCommands[constants.WALLET_rivineG]),
						'encryptionpassword': this.props.walletPassword,
					}

					//Grab input, spawn process, and write options.
					let rivinec = httpCommand(this.props.currentCommand, this.props.actions, this.props.commandHistory.size)
					rivinec.write(querystring.stringify(options))
					rivinec.end()
				...
				default:
					...
					rivinec = httpCommand(this.props.currentCommand, this.props.actions, this.props.commandHistory.size)
					rivinec.write(querystring.stringify({ 'encryptionpassword': this.props.walletPassword }))
					rivinec.end()
					this.props.actions.setWalletPassword('')
					break
				}
